...
public class ExcelServiceImpl implements ExcelService {
    ...
    public void generateExcel(final String userName,
            ...
            final List<Date> dateList, final String resultPath)
        ...
        try {
            ...
            // header name
            final ConfigurationFileParser configurationFileParser =
            ...
            final int headerRow =
                    getStartHeader(sheet, columnSize, propertyMap);

            final Map<String, Map<String, String>> eventKeyValue =
                    new HashMap<String, Map<String, String>>();
            for (final Map.Entry<String, List<DateTime>> entry : excelData
                    .entrySet()) {
            ...
            setHeaderValue(sheet,
                    getValueFromKeyAsString(eventKeyValue, CLI.toLowerCase()),
                    userName,
        ...
        for (final String string : eventData) {
            ...
            if (keyValue != null && keyValue.length == 2) {
                ...
                if (keyValue[0].charAt(0) == CHAR_AT_THE_RATE
                    ...
                    map.replace(lastKey,
                            map.get(lastKey).concat(" ").concat(keyValue[0]));
                }
            }
        }

        final long duration = new Date(entry.getValue().get(1).getValue())
                .getTime()
                - new Date(entry.getValue().get(0).getValue()).getTime();

        final long diffInMinutes = TimeUnit.MILLISECONDS.toMinutes(duration);

        map.put(ENDDATE.toLowerCase(), CalendarConstant.TABLE_DATE_FORMAT
                .format(new Date(entry.getValue().get(1).getValue())));
        map.put(STARTDATE.toLowerCase(), CalendarConstant.TABLE_DATE_FORMAT
                .format(new Date(entry.getValue().get(0).getValue())));
        map.put(STAFF.toLowerCase(), userName);
        map.put(WORKEDHOURS.toLowerCase(), diffInMinutes / 60 + COL_SPLITTER
                + String.format("%02d", diffInMinutes % 60));
                ...
                if (row != null) {
                    ...
                    if (cell != null && cell.getStringCellValue() != null
                        ...
                        case FROM_HEADER:
                            valueCell = row.getCell(j + 1);
                            valueCell.setCellValue(
                                    CalendarConstant.EXCEL_HEADER_DATE_FORMAT
                                            .format(startDate));
                            break;
