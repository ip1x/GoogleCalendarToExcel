...
public class EventTitleParser {
	...
	final String[] data = eventSummary.split("(?<=\\G(\\w+(?!\\w+)|:|CLI:|PRJ:|ACT:|TKT:|WBS:|@|%))\\s*");
	...
	for (int count = 0; count < data.length; count++) {
		...
		if (String.valueOf(CHAR_AT_THE_RATE).equals(data[count])
		    ...
		    settingEventValueToMap(data, eventMap, key, notKey, count);
		}
	    } catch (final Exception e) {
		throw new InvalidEventException();
	    }
    ...
    private static void settingEventValueToMap(final String[] data, final Map<String, String> map, final String key,
