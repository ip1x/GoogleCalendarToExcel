...
import static com.google.calendar.constant.CalendarConstant.ENDED_ON;
import static com.google.calendar.constant.CalendarConstant.STARTED_ON;
...
public class EventTitleParser {
	...
	// eventSummary = "CLI:aaaaJ:pppp PRJ: pppp TKT:nnnn WBS:wwww ACT:aaaa
	// @:cccc %:bbbb"; // valid
	// eventSummary = "CLI:aaaa TKT:nnnn PRJ:pppp WBS:wwww ACT:aaaa @:cccc
	// %:bbbb"; // valid
	// eventSummary = ":CLI"; // starting with ":" //invalid
	// eventSummary = "CLI: PRJ:sagicor"; // valid
	// eventSummary = "CLI:aaaaPRJ:pppp TKT:nnnn WBS:wwww ACT:aaaa @:cccc
	// %:bbbb"; // valid
	// eventSummary = "CLI:aaaaPRJ:pppp PRJ:pppp TKT:nnnn WBS:wwww ACT:aaaa
	// @:cccc %:bbbb"; // valid
	// eventSummary = "PRJ:propulsor %: 80 ACT: test on site @:Imola"; //
	// valid
	// eventSummary = "PRJ:propulsor %:80"; // valid
	// eventSummary = "CLI :aaaa PRJ:pppp TKT:nnnn WBS:wwww ACT:aaaa @:cccc
	// %:bbbb"; // space between ":" and "CLI" // invalid
	// eventSummary = "CLI:aaaa PRJ:pppp TKT:nnnn WBS:wwww rohit:aaaa @:cccc
	// %:bbbb"; // invalid tag "rohit" // invalid
	// eventSummary = "CLI:aaaa PRJ:pppp TKT:nnnn WBS:wwww rohit:aaaa @:cccc
	// %:bbbb"; // valid tag "rohit" // valid
	// eventSummary = "CLI:aaaa PRJ:pppp TKT:nnnn WBS:wwww ACT:aaaa @:cccc
	// %:"; // no value // valid
	// eventSummary = "CLI:aaaa %: TKT:nnnn WBS:wwww ACT:aaaa @:cccc
	// PRJ:pppp"; // positional // valid
	// eventSummary = "cli:aaaa %: TkT:nnnn Wbs:wwww ACT:aaaa @:cccc
	// PRJ:pppp"; // case insensitive // valid
	...
	eventMap.put(ENDED_ON.toLowerCase(),
    ...
    // private void checkForInvalidEvent(final String[] data) throws
    // InvalidEventException {
    // if ((data.length == 1) || ((data.length == 2) &&
    // !String.valueOf(CHAR_AT_THE_RATE).equals(data[0])
    // && !String.valueOf(CHAR_MODULUS).equals(data[0]))) {
    // throw new InvalidEventException();
    // }
    // if (data[0].equals(COL_SPLITTER) || data[data.length -
    // 1].equals(COL_SPLITTER)) {
    // throw new InvalidEventException();
    // }
    // }

    // private static void parsingEventForValue(final String[] data, final
    // Map<String, String> map, final String key,
    // final String notKey, final int count) {
    // if (map.get(key) != null) {
    // if (notKey != null) {
    // map.replace(key,
    // map.get(key).concat(COL_SPLITTER).concat(data[count].trim()));
    // } else {
    ...
    // CheckValidityOfEvent(final Event event, final String userName,
    ...
    // throw new InvalidEventException();
    // } else {
    // final Map<String, Map<String, String>> eventKeyValue = new
    // HashMap<>();
    // map.put(ENDDATE.toLowerCase(),
    // CalendarConstant.TABLE_DATE_FORMAT.format(new
    // Date(event.getEnd().getDateTime().getValue())));
    // map.put(STARTDATE.toLowerCase(),
    // CalendarConstant.TABLE_DATE_FORMAT.format(new
    // Date(event.getStart().getDateTime().getValue())));
