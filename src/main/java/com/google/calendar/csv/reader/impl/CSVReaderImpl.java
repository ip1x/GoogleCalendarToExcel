...
/**
 * This class is used to read the csv file and convert the csv file data into
 * map object
 *
 * @author DAMCO
 */
public class CSVReaderImpl implements CSVReader {
     ...
     *
     * @param request
     *            HttpServletRequest object for input CSV file.
     * @param response
     *            HttpServletResponse object,contain exception message
     *            if occurred
     * @param bufferReader
     *            A buffer to store data of CSV file.
	...
	if ((line = bufferReader.readLine()) != null) {
	    ...
	    // Splits the input string in CSV file.
	    final String[] inputFileArgument = line.split(" ");
	    for (final String input : inputFileArgument) {
		// String is further split with ":" with limit 2.
		final String[] inputTagValue = input.split(CalendarConstant.COL_SPLITTER, 2);
		if ((inputTagValue != null) && (inputTagValue.length == 2)) {
		    // If after split, we have 2 strings then put it in MAP with
		    // 1st string as KEY and 2nd one as VALUE.
		    inputMap.put(inputTagValue[0].trim(), inputTagValue[1].trim());
		    lastKey = inputTagValue[0].trim();
		}
		if ((inputTagValue != null) && (inputTagValue.length == 1)) {
		    // if after split, we have only one string then Append the
		    // string with the EXISTING VALUE of the KEY in
		    // the MAP with " "(space) between them.
		    inputMap.replace(lastKey, inputMap.get(lastKey).concat(" ").concat(inputTagValue[0].trim()));
		}
	    }
