...
public class UploadServlet extends HttpServlet {
    ...
    public void doPost(final HttpServletRequest request,
        ...
        try {
            ...
            final Map<String, String> inputMap =
                    csvReader.readCSV(request, response);

            final List<String> calendarName =
                    Arrays.asList(inputMap.get(CalendarConstant.CALENDAR)
                            .split(CalendarConstant.COMMA_SPLITTER));
            final String templatePath =
                    inputMap.get(CalendarConstant.TEMPLATE) != null
            ...
            final String inOutPath =
                    inputMap.get(CalendarConstant.INOUTMAP) != null
            ...
            final CalendarService calendarService =
                    (CalendarService) ServiceFactory
            ...
            do {
                final CalendarList calendarList = service.calendarList().list()
                        .setPageToken(pageToken).execute();
                final List<CalendarListEntry> listItems =
                            ...
                            for (final Event event : items) {
                                try {
                                    final Map<String, String> summaryMap =
                                            getProjecAndClientName(
                                                    event.getSummary());
                                    if ((clientName.contains(
                                            summaryMap.get("CLI").trim())
                                            || clientName.isEmpty())
                                            && (projectName
                                                    .contains(summaryMap
                                                            .get("PRJ").trim())
                                    ...
                                    logger.info(
                                            "Event with uncompiled name is found");
            ...
            dateList.add(toDate);
            final ExcelService excelService = (ExcelService) ServiceFactory
                    .getInstance(ExcelService.class);
            excelService.generateExcel(userName, projectName, clientName,
                    templatePath, inOutPath, excelData, dateList, resultPath);

            if (firstEvent != null) {
                final File file = new File(
                        CalendarConstant.RESULT_FILE_NAME.equals(resultPath)
    ...
    /**
     * Used to fetch CLIENT and PROJECT details from the calendar event. It also
     * checks whether event is in valid format or not.
     *
     * @param summary event title to be parsed for details.
     * @return
     */
    private Map<String, String> getProjecAndClientName(final String summary) {
        ...
        for (final String string : eventData) {
            ...
            if (keyValue != null && keyValue.length == 2) {
                ...
                if ((keyValue[0].charAt(0) == CHAR_AT_THE_RATE)
                        || (keyValue[0].charAt(0) == CHAR_MODULUS)) {
